...
from iminuit import minimize as minuit # PZ
...
def get_minimum(cosmo, data, command_line, covmat):
    ...
    # PZ hack for pybird on fixed cosmology: no re-evaluation of the cosmology-dependent part
    if data.fix_cosmo: data.update_cosmo_arguments() 
    ...
    #                      )
    result = minuit(chi2_eff, parameters, args = (cosmo,data), bounds=bounds, tol=command_line.minimize_tol) # minuit has internal stop criterion
    ...
    print ('chi2: ', chi2_eff(result.x, cosmo, data)) # PZ: this is mainly to put the derived_lkl parameters on the bestfit
...
def chi2_eff(params, cosmo, data, bounds=False):
    ...
    if not data.fix_cosmo: data.update_cosmo_arguments() # PZ added
    # Compute loglike value for the new parameters
    chi2 = -2.*compute_lkl(cosmo, data)
    if data.fix_cosmo: data.need_cosmo_update = False # PZ added

    #FK: use list-comprehension so that the parameter values are distinguishable from step to step
    # print('In minimization: ',chi2,' at ',[param for param in params]) # PZ commented
...
def get_fisher_matrix_PZ(cosmo, data, command_line, inv_fisher_matrix, minimum=0): 
    
    fiducial = {}
    parameter_names = data.get_mcmc_parameters(['varying'])
    read_args_from_bestfit(data, command_line.bf) # this stores command_line.bf in data.mcmc_parameters['last_accepted']
    for elem in parameter_names: 
        fiducial[elem] = data.mcmc_parameters[elem]['last_accepted']

    step = {}
    for i, a in enumerate(parameter_names): 
        step[a] = inv_fisher_matrix[i][i]**.5

    print ('computing lkl on fiducial...')
    for elem in parameter_names: data.mcmc_parameters[elem]['current'] = fiducial[elem]
    data.update_cosmo_arguments()
    chi2_fid = -2.*compute_lkl(cosmo, data)
    if data.fix_cosmo: data.need_cosmo_update = False

    def get_chi2(params):
        for elem in parameter_names: data.mcmc_parameters[elem]['current'] = params[elem] 
        if not data.fix_cosmo: data.update_cosmo_arguments() # this takes data.mcmc_parameters['current'] and put it in classy
        chi2 = -2.*compute_lkl(cosmo, data)
        return chi2

    print ('computing lkl derivatives around fiducial...')
    nparams = len(parameter_names)
    fisher = np.zeros(shape=(nparams, nparams))
    for i, a in enumerate(parameter_names): 
        pp, pm = fiducial.copy(), fiducial.copy()
        pp[a] += step[a]
        pm[a] -= step[a]
        fisher[i,i] = 0.5 * ( get_chi2(pp) + get_chi2(pm) - 2 * chi2_fid ) / step[a]**2
        for j, b in enumerate(parameter_names): 
            if i > j: 
                ppp, ppm, pmp, pmm = fiducial.copy(), fiducial.copy(), fiducial.copy(), fiducial.copy()
                ppp[a] += step[a]; ppp[b] += step[b]
                ppm[a] += step[a]; ppm[b] -= step[b]
                pmp[a] -= step[a]; pmp[b] += step[b]
                pmm[a] -= step[a]; pmm[b] -= step[b]
                fisher[i,j] = 0.5 * ( get_chi2(ppp) + get_chi2(pmm) - get_chi2(ppm) - get_chi2(pmp) ) / ( 4.* step[a] * step[b] )

    for i, a in enumerate(parameter_names): 
        for j, b in enumerate(parameter_names): 
            if i < j: fisher[i,j] = fisher[j,i]

    # fisher += 1.e-6
    inv_fisher_matrix = np.linalg.inv(fisher)

    # rg = np.random.default_rng(0)
    # samps = rg.multivariate_normal(fiducial_in_array, inv_fisher_matrix, size=int(1e5))

    print ('error bars: ')
    for i, a in enumerate(parameter_names): 
        print (a, ': ', inv_fisher_matrix[i,i]**.5)
